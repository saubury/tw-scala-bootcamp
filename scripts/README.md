...
4. Collections
    ...
    *Optional* - See explanation from Dick Wall [here](https://drive.google.com/open?id=1dGKnQThxeiinDuwmFaw7tIsrjaaC1PSN)
    ...
    HINT: Try pressing TAB after typing an object name and member selection dot that follows (e.g. `lines.`) for a list of available members. Experiment with tab completion elsewhere in the REPL.
...
5. Moving beyond the REPL: writing, compiling and running programs
...
6. Using a build tool (sbt)
    ...
    With scala projects, it is common to use the [Simple build tool](https://www.scala-sbt.org) commonly known as `sbt`. This is akin to maven in the java world or rake in the ruby world.
    ...
    You will find `sbt` installed in the docker image. To get started, we need to make a build configuration called `build.sbt`
...
7. Reading Input
    ...
    *Note* scala string interpolation syntax with the quotes prefixed with `s`
    Scala supports custom interpolators but we won't go into depth about that here.
...
8. Splitting strings
    ...
    HINT: You can ask `sbt` to automatically re-run your tests every time you save a change by prepending a tilde to "test", as in `sbt ~test`.

9. Use a sliding window function

    We now need a method to calculate the average over a sliding window, so let's start with a test for this:
    ...
    Notice it fails with `getMovingAvg is not a member of object HelloWorld3`.

    To make the test pass, we can then create a method to perform a series of maps:
